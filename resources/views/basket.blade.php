...
<div class="sizer">
  ...
  @if(count($orders)==0)
              <center> <h2>Товари ще не додані в кошик</h2></center>
          @else
...
<td class="product-id" style="display:none;">
	<input id="itemid" type="hidden" value="{{$order->item_id}}">
</td>
					<td class="product-remove">
						<a onclick="del_order({{$order->item_id}})" href="#" class="remove" title="Удалить товар" data-product_id="4410" data-product_sku="LE12">×</a>					</td>

					<td class="product-thumbnail">
						<a href="#"><img style="max-width:90px;"width="90" height="90" src="{{$order->img}}" class="attachment-shop_thumbnail wp-post-image" alt="loving-earth-cold pressed coconut-oil"></a>					</td>

					<td class="product-name">
						<a href="#">{{$order->title}} </a>					</td>

					<td class="product-price">
						<span id="orderprice" class="amount">{{$order->price}}</span>					</td>

					<td class="product-quantity">
						<div class="quantity"><input onchange="changeamt(this,'{{$order->item_id}}','{{$order->price}}');" id="kstcart{{$order->item_id}}" type="number" style="width:50px;"step="1" min="0" max="99" name="cart[ea96efc03b9a050d895110db8c4af057][qty]" value="{{$order->amount}}" title="К-сть" class="input-text qty text kstcart" size="4"></div>					</td>
			...
			<td colspan="6" class="actions">
				...
				<input type="submit" class="button" name="update_cart" value="Оновити кошик"> <input type="submit" onclick="gocheckout();" class="checkout-button button alt wc-forward" name="proceed" value="Замовити продукти">
